...
/*
  ...
  */
static int jx9Builtin_zip_entry_reset_read_cursor(jx9_context *pCtx, int nArg, jx9_value **apArg)
{
	SyArchiveEntry *pEntry;
	if( nArg < 1 || !jx9_value_is_resource(apArg[0]) ){
		/* Missing/Invalid arguments */
...
static void WinVfs_Username(jx9_context *pCtx)
{
	WCHAR zUser[1024];
	DWORD nByte;
	BOOL rc;
	...
	if( !rc ){
		...
		if( zName == 0 ){
			...
			jx9_result_string(pCtx, zName, -1/*Compute length automatically*/); /* Will make it's own copy */
...
static const jx9_vfs sWinVfs = {
	...
	0, /* int (*xUid)(void) */
	0, /* int (*xGid)(void) */
	WinVfs_Username,    /* void (*xUsername)(jx9_context *) */
	0 /* int (*xExec)(const char *, jx9_context *) */
};
/* Windows file IO */
...
JX9_PRIVATE sxi32 jx9RegisterIORoutine(jx9_vm *pVm)
{
	...
	static const jx9_builtin_func aVfsFunc[] = {
		...
		{"zip_entry_reset_read_cursor", jx9Builtin_zip_entry_reset_read_cursor}, 
